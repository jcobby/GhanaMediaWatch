...
interface BusinessState {
  /**
   * Interest overrides, keyed by business id.
   *
   * Seeded interests live in `BUSINESSES` and are never mutated — a demo that
 ...
 */
export function effectiveInterests(business: BusinessAccount): IncidentCategory[] {
  return useBusinessStore.getState().interestOverrides[business.id] ?? business.interests;
}

/** Reactive form of {@link effectiveInterests}, for use inside components. */
export function useEffectiveInterests(business: BusinessAccount): IncidentCategory[] {
  return useBusinessStore((s) => s.interestOverrides[business.id]) ?? business.interests;
}
